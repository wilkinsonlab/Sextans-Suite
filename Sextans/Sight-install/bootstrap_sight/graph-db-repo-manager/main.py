...
# these are set in the docker-compose (prefix is in the installer)
...
def create_repository(graphdb_url, repo_id, repo_description):
      # parameters passed differ for each repo - cde or fdp
    ...
    if not check_repository(graphdb_url, repo_id):
        ...
        file = open("config.ttl", "w")  # overwrite the last one
...
def main(graphdb_url):
    ...
    '''
    repo_id = FDP_PREFIX + "-sextans-sight"
    repo_description = "Repository to store Sextans Sight's metadata RDF documents"
    create_repository(graphdb_url, repo_id, repo_description)
    print(repo_id + " Created")
