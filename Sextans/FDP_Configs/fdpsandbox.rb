...
schema_base = './ERDERA_Base/Schemas'
...
#   next unless resource.name == 'Distribution'

#   puts "resource name: #{resource.name}, payload: #{JSON.pretty_generate(resource.to_api_payload)}"
#   break
# end

biobank = FDP::Resource.new(client: fdp,
                            schemas: [fdp.list_current_schemas['Biobank']['uuid']], # link to the Biobank schema we created
                            description: 'A biobank resource definition for testing',
                            prefix: 'biobank',
...
puts "\nResources:"
resources.each do |resource|
  next unless resource.name == 'ERDERA BioBank'

  puts "resource name: #{resource.name}, uuid: #{resource.uuid}, prefix: #{resource.prefix}, schemas: #{resource.schemas.join(', ')}"
end

# edit to change name
biobank.name = 'ERDERA BioBank Updated'
warn "trying to overwrite existing resource with new name: #{biobank.name} and same UUID: #{biobank.uuid}"
biobank.write_to_fdp(client: fdp)

puts 'Final Resource To Payload:'
resources.each do |resource|
  next unless resource.name == 'ERDERA BioBank'
...
schema = FDP::Schema.new(client: fdp,
